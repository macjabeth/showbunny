...
class KittySan {
  constructor() {
    ...
    this.elements.streamClose = this.elements.streamContainer.getElementsByClassName(
      "close-btn"
    )[0];
  }
  ...
  paintSearchResults(data) {
    if (this.elements.searchResults.children.length) {
      for (const child of this.elements.searchResults.children) {
        child.classList.remove("fadeIn", "zoomInUp", "delay-1s");
  ...
  paintEpisodeSelect() {
    ...
    ["episode", "season"].forEach(value => {
      ...
      this.elements.mediaSelect.insertBefore(
        selectList,
        this.elements.mediaSelect.childNodes[0]
      );
    });
  }
      ...
      bunny
        ...
        .then(data => {
          ...
          selectSeason.onchange = () => {
            ...
            const episodes =
              data.seasons[parseInt(selectedSeason) - 1].episode_count;
          ...
          this.elements.streamEpisodeBtn.onclick = () => {
            ...
            if (selectedSeason === "0" || selectedEpisode === "0") {
              ...
              window.open(
                `https://ololo.to/s/${result.name} s${selectedSeason.padStart(
                  2,
                  "0"
                )}e${selectedEpisode.padStart(2, "0")} --new --strict`
              );
            }
          };
